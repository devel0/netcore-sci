...
namespace SearchAThing.Sci.Tests
{
    public partial class Arc3DTests_torefact
    {
        ...
        public void Arc3DTest_001()
        {
            ...
            var ip_circ = c.Intersect(1e-4, seg_i, onlyPerimeter: true, lineSegmentMode: false);
            var ip_arc = arc.Intersect(1e-2, seg_i, onlyPerimeter: true, lineSegmentMode: true, arcSegmentMode: true);
            ...
            Assert.True(c.Intersect(1e-4, seg_i2, onlyPerimeter: true, lineSegmentMode: true).Count() == 0);
            Assert.True(arc.Intersect(1e-4, seg_i2, onlyPerimeter: true, lineSegmentMode: true, arcSegmentMode: true).Count() == 0);

            // segment not intersecting, but line intersecting arc
            Assert.True(c.Intersect(1e-4, seg_i2, onlyPerimeter: true, lineSegmentMode: false, arcSegmentMode: true).Count() == 1);
            Assert.True(c.Intersect(1e-4, seg_i2, onlyPerimeter: true, lineSegmentMode: false, arcSegmentMode: true).First().EqualsTol(1e-4, ip_arc.First()));

            // segment intresecting circle, but not arc
            var seg_e = new Line3D(2.4523, 9.6971, -1.8004, 4.6142, -.0631, -2.0519);
            Assert.True(c.Intersect(1e-4, seg_e, onlyPerimeter: true, lineSegmentMode: false).Count() == 1);
            Assert.True(c.Intersect(1e-4, seg_e, onlyPerimeter: true, lineSegmentMode: false).First().EqualsTol(1e-2, 3.53, 4.82, -1.93));
            Assert.True(arc.Intersect(1e-4, seg_e, onlyPerimeter: true, lineSegmentMode: true, arcSegmentMode: true).Count() == 0);
        ...
        public void Arc3DTest_007()
        {
            ...
            Assert.True(arc.Intersect(1e-6, arc.CS.Move(arc.CS.BaseZ * 1000), onlyPerimeter: true, inArcAngleRange: true).Count() == 0);
