...
namespace SearchAThing.Sci.Tests
{
    public partial class LoopTests
    {
        ...
        public void LoopTest_0014()
        {
            ...
            var sub = loopYellow.Boolean(tol, loopGreen, Loop.BooleanMode.Difference, outdxf2).ToList();
            ...
            sub[0].Area.AssertEqualsTol(tol, 361.8427445113067);
            sub[0].Length.AssertEqualsTol(tol, 94.69225675761145);

            sub[1].Area.AssertEqualsTol(tol, 181.0339736153745);
            sub[1].Length.AssertEqualsTol(tol, 73.22649595561018);

            sub[2].Area.AssertEqualsTol(tol, 123.0062807844365);
            sub[2].Length.AssertEqualsTol(tol, 58.998642980826205);

            sub[3].Area.AssertEqualsTol(tol, 94.60509880947234);
            sub[3].Length.AssertEqualsTol(tol, 39.638105742989445);

            sub[4].Area.AssertEqualsTol(tol, 299.1853730731076);
            sub[4].Length.AssertEqualsTol(tol, 77.28269070361564);

            sub[5].Area.AssertEqualsTol(tol, 11.728056836031051);
            sub[5].Length.AssertEqualsTol(tol, 20.709388210898226);
